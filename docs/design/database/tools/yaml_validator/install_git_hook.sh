...
# 検証スクリプトのパス
VALIDATOR_SCRIPT="docs/design/database/tools/yaml_validator/validate_yaml_format.py"

if [ ! -f "$VALIDATOR_SCRIPT" ]; then
    echo -e "${RED}エラー: 検証スクリプト $VALIDATOR_SCRIPT が見つかりません。${NC}"
    exit 1
fi
...
for FILE in $YAML_FILES; do
    ...
    python "$VALIDATOR_SCRIPT" --table "$TABLE_NAME"
...
if [ $FAILED -eq 1 ]; then
    ...
    echo -e "${YELLOW}詳細なガイドラインは docs/design/database/tools/yaml_validator/README_REQUIRED_SECTIONS.md を参照してください。${NC}"
