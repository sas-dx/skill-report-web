...
# -*- coding: utf-8 -*-
"""
テーブル定義書生成スクリプト v7 (レイアウト改良版)
- 桁数・PK・FK情報を追加したカラム定義表
- パラメータ指定による個別テーブル生成対応
- 設定ファイル駆動によるハードコード排除
- 改版履歴の自動反映機能
- エラーハンドリングの改善
"""
...
"""
    ...
    def generate_files(self, table_names: List[str] = None, output_dir: str = None):
        ...
        print(f"🚀 テーブル定義書生成スクリプト v7 (レイアウト改良版)")
        print("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="テーブル定義書生成スクリプト v7 (レイアウト改良版)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用例:
  # 全テーブル生成
  python3 create_table_definitions_v7.py
  
  # 個別テーブル生成
  python3 create_table_definitions_v7.py --table MST_Employee
  python3 create_table_definitions_v7.py --table MST_Role,MST_Permission
  
  # カテゴリ別生成
  python3 create_table_definitions_v7.py --category マスタ系
  
  # 出力先指定
  python3 create_table_definitions_v7.py --table MST_Employee --output-dir custom/
        """
    )
