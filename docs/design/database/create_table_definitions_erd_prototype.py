...
# -*- coding: utf-8 -*-
"""
テーブル定義書生成スクリプト（ERD機能付きプロトタイプ）

プロフィール管理テーブル専用のERD生成機能を追加したプロトタイプ版
"""
        ...
        for table in sorted(related_tables[1]):
            ...
            for rel in relationships:
                if rel['to_table'] == table:
                    ...
                    erd_content += f"    {target_table} }o--|| {table} : \"{desc}\"\n"
            
            relationships = self.get_foreign_key_relationships(table)
            for rel in relationships:
                if rel['to_table'] == target_table:
                    desc = rel['description'] or "参照"
                    erd_content += f"    {table} }o--|| {target_table} : \"{desc}\"\n"
            ...
            for level1_table in sorted(related_tables[1]):
                ...
                for rel in relationships:
                    if rel['to_table'] == level1_table:
                        ...
                        erd_content += f"    {table} }o--|| {level1_table} : \"{desc}\"\n"
                        break  # 1つの関係のみ表示
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="テーブル定義書ERD生成プロトタイプ",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用例:
  # MST_EmployeeのERD生成テスト
  python3 create_table_definitions_erd_prototype.py --table MST_Employee
  
  # プロフィール管理テーブル全体のERD生成テスト
  python3 create_table_definitions_erd_prototype.py --profile-tables
        """
    )
