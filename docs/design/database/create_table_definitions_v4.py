...
def parse_extended_table_list_md(file_path: str) -> List[Dict[str, str]]:
    ...
    try:
        ...
        for line in lines:
            ...
            # テーブルの開始を検出
            if '| テーブルID |' in line and 'カテゴリ' in line:
                in_table = True
                header_found = True
                continue
            ...
            if in_table and line.startswith('|') and header_found:
                ...
                if len(columns) >= 21:
                    ...
                    table_info = {
                        ...
                        'category': category,
                        'function_category': columns[5].strip() if len(columns) > 5 else '',
                        ...
                        'initial_data_count': columns[9].strip() if len(columns) > 9 else '10件',
                        'monthly_increase': columns[10].strip() if len(columns) > 10 else '100件',
                        'yearly_increase': columns[11].strip() if len(columns) > 11 else '1,200件',
                        'five_year_estimate': columns[12].strip() if len(columns) > 12 else '6,010件',
                        'select_response_time': columns[13].strip() if len(columns) > 13 else '10ms以内',
                        'insert_response_time': columns[14].strip() if len(columns) > 14 else '50ms以内',
                        'update_response_time': columns[15].strip() if len(columns) > 15 else '50ms以内',
                        'delete_response_time': columns[16].strip() if len(columns) > 16 else '100ms以内',
                        'personal_info': columns[17].strip() if len(columns) > 17 else 'なし',
                        'confidential_level': columns[18].strip() if len(columns) > 18 else '低',
                        'encryption_required': columns[19].strip() if len(columns) > 19 else '不要',
                        'archive_condition': columns[20].strip() if len(columns) > 20 else '無効化から3年経過',
                        'screens': columns[21].strip() if len(columns) > 21 else ''
                    }
                    
                    tables.append(table_info)
            
            # テーブル終了の検出（次のセクションの開始）
            if in_table and line.startswith('##') and ('カテゴリ別特性' in line or '運用指針' in line):
                break
...
def main():
    """メイン処理"""
    print("🚀 テーブル定義書一括再作成を開始します（v4.0 - 拡張版対応）...")
