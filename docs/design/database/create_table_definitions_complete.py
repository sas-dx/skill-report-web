...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="テーブル定義書生成スクリプト（完全版）",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用例:
  # 全テーブルの定義書を生成
  python3 create_table_definitions_complete.py --all
  
  # 特定のテーブルの定義書を生成
  python3 create_table_definitions_complete.py --tables MST_Employee MST_Department
  
  # プロフィール管理テーブルのみ生成
  python3 create_table_definitions_complete.py --category profile
        """
    )
