...
### 2.1 概要・目的
MST_CertificationRequirement（資格要件マスタ）は、資格要件マスタに関する情報を管理するテーブルです。

### 2.2 関連API
- [API-033](../../api/specs/) - 関連API

### 2.3 関連バッチ
- [BATCH-029](../../batch/specs/) - 関連バッチ
...
| 21 | column_21 | カラム21 | VARCHAR | 100 | ○ | - | - | NULL | カラム21の説明 |
| 22 | column_22 | カラム22 | VARCHAR | 100 | ○ | - | - | NULL | カラム22の説明 |
| 23 | column_23 | カラム23 | VARCHAR | 100 | ○ | - | - | NULL | カラム23の説明 |
| 24 | column_24 | カラム24 | VARCHAR | 100 | ○ | - | - | NULL | カラム24の説明 |
| 25 | column_25 | カラム25 | VARCHAR | 100 | ○ | - | - | NULL | カラム25の説明 |
...
| 制約名 | 制約種別 | カラム | 制約内容 |
|--------|----------|--------|----------|
| pk_mst_certificationrequirement | PRIMARY KEY | id | 主キー制約 |
...
CREATE TABLE MST_CertificationRequirement (
    ...
    CONSTRAINT fk_mst_certificationrequirement_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_mst_certificationrequirement_created_by FOREIGN KEY (created_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_mst_certificationrequirement_updated_by FOREIGN KEY (updated_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT
