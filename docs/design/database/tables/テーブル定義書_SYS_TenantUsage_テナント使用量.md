...
### 2.1 概要・目的
SYS_TenantUsage（テナント使用量）は、テナント使用量に関する情報を管理するテーブルです。

### 2.2 関連API
- [API-025](../../api/specs/) - 関連API

### 2.3 関連バッチ
- [BATCH-018-01](../../batch/specs/) - 関連バッチ
...
CREATE TABLE SYS_TenantUsage (
    ...
    CONSTRAINT fk_sys_tenantusage_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
