...
### 2.1 概要・目的
SYS_BackupHistory（バックアップ履歴）は、バックアップ履歴に関する情報を管理するテーブルです。

### 2.2 関連API
- [API-020](../../api/specs/) - 関連API

### 2.3 関連バッチ
- [BATCH-013](../../batch/specs/) - 関連バッチ
...
CREATE TABLE SYS_BackupHistory (
    ...
    CONSTRAINT fk_sys_backuphistory_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_sys_backuphistory_created_by FOREIGN KEY (created_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_sys_backuphistory_updated_by FOREIGN KEY (updated_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT
