...
### 2.1 概要・目的
MST_Tenant（テナント管理）は、テナント管理に関する情報を管理するテーブルです。

### 2.2 関連API
- [API-025](../../api/specs/) - 関連API

### 2.3 関連バッチ
- [BATCH-018-01, BATCH-018-02](../../batch/specs/) - 関連バッチ
...
CREATE TABLE MST_Tenant (
    ...
    CONSTRAINT fk_mst_tenant_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_mst_tenant_created_by FOREIGN KEY (created_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_mst_tenant_updated_by FOREIGN KEY (updated_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT
