...
### 2.1 概要・目的
MST_SkillCategory（スキルカテゴリマスタ）は、スキルカテゴリマスタに関する情報を管理するテーブルです。

### 2.2 関連API
- [API-030](../../api/specs/) - 関連API

### 2.3 関連バッチ
- [BATCH-020](../../batch/specs/) - 関連バッチ
...
CREATE TABLE MST_SkillCategory (
    ...
    CONSTRAINT fk_mst_skillcategory_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
