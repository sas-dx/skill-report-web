...
### 2.1 概要・目的
MST_SkillHierarchy（スキル階層マスタ）は、スキル階層マスタに関する情報を管理するテーブルです。

### 2.2 関連API
- [API-007](../../api/specs/) - 関連API

### 2.3 関連バッチ
- [BATCH-005](../../batch/specs/) - 関連バッチ
...
| 制約名 | 制約種別 | カラム | 制約内容 |
|--------|----------|--------|----------|
| pk_mst_skillhierarchy | PRIMARY KEY | id | 主キー制約 |
...
CREATE TABLE MST_SkillHierarchy (
    ...
    CONSTRAINT fk_mst_skillhierarchy_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_mst_skillhierarchy_created_by FOREIGN KEY (created_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_mst_skillhierarchy_updated_by FOREIGN KEY (updated_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT
