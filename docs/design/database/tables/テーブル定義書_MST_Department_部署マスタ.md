...
### 2.1 概要・目的
MST_Department（部署マスタ）は、部署マスタに関する情報を管理するテーブルです。

### 2.2 関連API
- [API-006](../../api/specs/) - 関連API

### 2.3 関連バッチ
- [BATCH-004, BATCH-015](../../batch/specs/) - 関連バッチ
...
CREATE TABLE MST_Department (
    ...
    CONSTRAINT fk_mst_department_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_mst_department_created_by FOREIGN KEY (created_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_mst_department_updated_by FOREIGN KEY (updated_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT
