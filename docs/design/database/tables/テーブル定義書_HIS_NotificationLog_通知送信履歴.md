...
### 2.1 概要・目的
HIS_NotificationLog（通知送信履歴）は、通知送信履歴に関する情報を管理するテーブルです。

### 2.2 関連API
- [API-029](../../api/specs/) - 関連API

### 2.3 関連バッチ
- [BATCH-019-04](../../batch/specs/) - 関連バッチ
...
| 制約名 | 制約種別 | カラム | 制約内容 |
|--------|----------|--------|----------|
| pk_his_notificationlog | PRIMARY KEY | id | 主キー制約 |
...
CREATE TABLE HIS_NotificationLog (
    ...
    CONSTRAINT fk_his_notificationlog_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_his_notificationlog_created_by FOREIGN KEY (created_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_his_notificationlog_updated_by FOREIGN KEY (updated_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT
