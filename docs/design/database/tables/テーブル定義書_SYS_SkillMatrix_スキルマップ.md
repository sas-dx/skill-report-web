# テーブル定義書：SYS_SkillMatrix（スキルマップ）
...
### 2.1 概要・目的
SYS_SkillMatrix（スキルマップ）は、スキルマップに関する情報を管理するテーブルです。

### 2.2 関連API
- [API-019](../../api/specs/) - 関連API

### 2.3 関連バッチ
- [BATCH-006](../../batch/specs/) - 関連バッチ
...
CREATE TABLE SYS_SkillMatrix (
    ...
    CONSTRAINT fk_sys_skillmatrix_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_sys_skillmatrix_created_by FOREIGN KEY (created_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_sys_skillmatrix_updated_by FOREIGN KEY (updated_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT
