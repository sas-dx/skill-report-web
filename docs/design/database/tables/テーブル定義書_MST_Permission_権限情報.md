...
### 2.1 概要・目的
MST_Permission（権限情報）は、権限情報に関する情報を管理するテーブルです。

### 2.2 関連API
- [API-003, API-004](../../api/specs/) - 関連API

### 2.3 関連バッチ
- [BATCH-003](../../batch/specs/) - 関連バッチ
...
CREATE TABLE MST_Permission (
    ...
    CONSTRAINT fk_mst_permission_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_mst_permission_created_by FOREIGN KEY (created_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_mst_permission_updated_by FOREIGN KEY (updated_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT
