...
### 2.1 概要・目的
SYS_IntegrationConfig（外部連携設定）は、外部連携設定に関する情報を管理するテーブルです。

### 2.2 関連API
- [API-028, API-029](../../api/specs/) - 関連API

### 2.3 関連バッチ
- [BATCH-019-03](../../batch/specs/) - 関連バッチ
...
| 制約名 | 制約種別 | カラム | 制約内容 |
|--------|----------|--------|----------|
| pk_sys_integrationconfig | PRIMARY KEY | id | 主キー制約 |
...
CREATE TABLE SYS_IntegrationConfig (
    ...
    CONSTRAINT fk_sys_integrationconfig_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
