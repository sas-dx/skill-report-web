...
### 2.1 概要・目的
TRN_ProjectRecord（案件実績）は、業務トランザクションデータを管理するテーブルです。日々の業務処理で発生するデータを格納します。
...
CREATE TABLE TRN_ProjectRecord (
    ...
    CONSTRAINT fk_trn_projectrecord_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_trn_projectrecord_created_by FOREIGN KEY (created_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_trn_projectrecord_updated_by FOREIGN KEY (updated_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT
