...
### 2.1 概要・目的
SYS_SkillIndex（スキル検索インデックス）は、スキル検索インデックスに関する情報を管理するテーブルです。

### 2.2 関連API
- [API-011](../../api/specs/) - 関連API
...
CREATE TABLE SYS_SkillIndex (
    ...
    CONSTRAINT fk_sys_skillindex_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_sys_skillindex_created_by FOREIGN KEY (created_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_sys_skillindex_updated_by FOREIGN KEY (updated_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT
