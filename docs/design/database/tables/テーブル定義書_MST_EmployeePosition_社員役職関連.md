# テーブル定義書：MST_EmployeePosition（社員役職関連）
...
### 2.1 概要・目的
MST_EmployeePosition（社員役職関連）は、社員役職関連に関する情報を管理するテーブルです。

### 2.2 関連API
- [API-021](../../api/specs/) - 関連API

### 2.3 関連バッチ
- [BATCH-026](../../batch/specs/) - 関連バッチ
...
| 制約名 | 制約種別 | カラム | 制約内容 |
|--------|----------|--------|----------|
| pk_mst_employeeposition | PRIMARY KEY | id | 主キー制約 |
...
CREATE TABLE MST_EmployeePosition (
    ...
    CONSTRAINT fk_mst_employeeposition_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_mst_employeeposition_created_by FOREIGN KEY (created_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_mst_employeeposition_updated_by FOREIGN KEY (updated_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT
