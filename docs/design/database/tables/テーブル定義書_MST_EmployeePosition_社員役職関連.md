...
> 詳細定義の変更は `table-details/MST_EmployeePosition_details.yaml` で行ってください。
...
|--------|------------|------------|------|------|
| uk_MST_EmployeePosition_employee_pos_primary | UNIQUE | employee_id, position_id, appointment_type, start_date |  | 社員・役職・任命区分・開始日一意制約 |
