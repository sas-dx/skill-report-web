...
### 2.1 概要・目的
WRK_BatchJobLog（一括登録ジョブログ）は、一時的な作業データを管理するテーブルです。バッチ処理や一括処理の際に使用されます。
...
CREATE TABLE WRK_BatchJobLog (
    ...
    CONSTRAINT fk_wrk_batchjoblog_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_wrk_batchjoblog_created_by FOREIGN KEY (created_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_wrk_batchjoblog_updated_by FOREIGN KEY (updated_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT
