...
### 2.1 概要・目的
MST_Employee（社員基本情報）は、社員基本情報に関する情報を管理するテーブルです。

### 2.2 関連API
- [API-005](../../api/specs/) - 関連API

### 2.3 関連バッチ
- [BATCH-004](../../batch/specs/) - 関連バッチ
...
CREATE TABLE MST_Employee (
    ...
    CONSTRAINT fk_mst_employee_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_mst_employee_created_by FOREIGN KEY (created_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_mst_employee_updated_by FOREIGN KEY (updated_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT
