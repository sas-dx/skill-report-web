...
### 2.1 概要・目的
HIS_AuditLog（監査ログ）は、監査ログに関する情報を管理するテーブルです。

### 2.2 関連API
- [API-022](../../api/specs/) - 関連API

### 2.3 関連バッチ
- [BATCH-003, BATCH-014](../../batch/specs/) - 関連バッチ
...
| 制約名 | 制約種別 | カラム | 制約内容 |
|--------|----------|--------|----------|
| pk_his_auditlog | PRIMARY KEY | id | 主キー制約 |
...
CREATE TABLE HIS_AuditLog (
    ...
    CONSTRAINT fk_his_auditlog_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_his_auditlog_created_by FOREIGN KEY (created_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_his_auditlog_updated_by FOREIGN KEY (updated_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT
