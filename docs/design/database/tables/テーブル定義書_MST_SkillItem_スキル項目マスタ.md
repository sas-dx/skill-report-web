...
### 2.1 概要・目的
MST_SkillItem（スキル項目マスタ）は、スキル項目マスタに関する情報を管理するテーブルです。

### 2.2 関連API
- [API-038](../../api/specs/) - 関連API

### 2.3 関連バッチ
- [BATCH-034](../../batch/specs/) - 関連バッチ
...
CREATE TABLE MST_SkillItem (
    ...
    CONSTRAINT fk_mst_skillitem_tenant FOREIGN KEY (tenant_id) REFERENCES MST_Tenant(tenant_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_mst_skillitem_created_by FOREIGN KEY (created_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_mst_skillitem_updated_by FOREIGN KEY (updated_by) REFERENCES MST_UserAuth(user_id) ON UPDATE CASCADE ON DELETE RESTRICT
