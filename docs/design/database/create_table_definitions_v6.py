...
"""
        ...
        # カラム定義
        md_content += "## 🗂️ カラム定義\n\n"
        md_content += "| カラム名 | 論理名 | データ型 | NULL | デフォルト | 説明 |\n"
        md_content += "|----------|--------|----------|------|------------|------|\n"
        
        # 基本カラム
        for col in self.common_columns['base_columns']:
            null_str = "○" if col.get('null', True) else "×"
            default_str = str(col.get('default', '')) if col.get('default') else ''
            md_content += f"| {col['name']} | {col['logical']} | {col['type']} | {null_str} | {default_str} | {col['description']} |\n"
        ...
        if details and 'business_columns' in details:
            for col in details['business_columns']:
                ...
                md_content += f"| {col['name']} | {col['logical']} | {col_type} | {null_str} | {default_str} | {col['description']} |\n"
    ...
    def generate_ddl(self, table_name: str, table_info: Dict[str, Any]) -> str:
        ...
        for col in self.common_columns['base_columns']:
            ...
            if not col.get('null', True):
                col_def += " NOT NULL"
            if col.get('default'):
                ddl_content += f" DEFAULT {col['default']}"
            if col.get('primary'):
                col_def += " PRIMARY KEY"
