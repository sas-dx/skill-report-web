...
| [SCR_CAR_Eval](specs/画面定義書_SCR_CAR_Eval_目標評価画面.md) | 目標評価画面 | 目標・キャリア管理 | 進捗管理・評価・フィードバック | 全ユーザー・上司 | 中 | CAR.2-TRACK.1, CAR.2-EVAL.1 | 目標進捗・評価管理 |
...
| [SCR_TRN_Master](specs/画面定義書_SCR_TRN_Master_研修マスタ画面.md) | 研修マスタ画面 | 研修・教育管理 | 研修プログラム・PDU管理 | 管理者 | 中 | TRN.3-PDU.1 | 継続教育管理 |
...
| [SCR_ADM_Master](specs/画面定義書_SCR_ADM_Master_マスタ管理画面.md) | マスタ管理画面 | システム管理 | 各種マスタデータ管理・同期 | システム管理者 | 低 | PRO.2-SYNC.1 | マスタデータ管理 |
...
graph TD
    ...
    %% 研修関連画面の遷移
    T --> TM[SCR_TRN_Master<br/>研修マスタ画面]
    ...
    H --> |システム管理者権限| AM[SCR_ADM_Master<br/>マスタ管理画面]
