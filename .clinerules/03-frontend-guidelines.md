...
この文書はNext.js 14 App Routerを中心としたフロントエンド設計ガイドラインを定義します。Server Components優先のアーキテクチャ、コンポーネント設計原則、状態管理パターン、パフォーマンス最適化、アクセシビリティ要件を提供し、モダンで保守性の高いフロントエンドアプリケーションの構築を支援します。技術スタック非依存の基本原則は00-core-rules.mdを参照し、このファイルではNext.js 14固有の実装パターンと最適化手法に焦点を当てています。
