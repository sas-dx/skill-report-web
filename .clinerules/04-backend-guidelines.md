...
この文書はバックエンド設計における汎用的なガイドラインを定義します。API設計規約（RESTful原則）、データベース設計原則、セキュリティ実装パターン、エラーハンドリング・ログ設計を提供し、スケーラブルで保守性の高いバックエンドシステムの構築を支援します。技術スタック非依存の基本原則を中心に記載し、特定フレームワーク固有の実装詳細は01-project-specific-rules.mdを参照してください。

## 基本設計原則

### 1. API設計原則
- **RESTful設計**: REST原則に従ったAPI設計
- **GraphQL対応**: 必要に応じたGraphQLの採用
- **API Gateway**: マイクロサービス環境でのAPI統合
- **型安全性**: 静的型付け言語による型安全なAPI実装

### 2. データアクセス層設計
- **ORM/ODM活用**: オブジェクトリレーショナルマッピングの活用
- **Repository Pattern**: データアクセスロジックの抽象化
- **トランザクション管理**: 複数操作の整合性保証
- **コネクション管理**: 効率的なデータベース接続管理

### 3. アーキテクチャ設計
- **レイヤードアーキテクチャ**: 責任の分離と依存関係の管理
- **マイクロサービス対応**: サービス間の疎結合設計
- **イベント駆動**: 非同期処理とスケーラビリティ
- **クリーンアーキテクチャ**: ビジネスロジックの独立性
